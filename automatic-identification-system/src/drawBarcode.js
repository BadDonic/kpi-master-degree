...
export const draw2of5Barcode = (barcode, size) => {
  ...
  canvas.width = N_COUNT * narrowWidth + W_COUNT * wideLineWidth + S_COUNT * narrowWidth + 2 * PADDING;
  canvas.height = lineHeight + 2 * PADDING;
  ...
  barcode.split('S').forEach(part => {
    part.split('').forEach((code, index) => {
      ...
      if (isBlack) {
        ctx.fillStyle = "black";
        ctx.fillRect(xPosition, PADDING, lineWidth, lineHeight);
      }
